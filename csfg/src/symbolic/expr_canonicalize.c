...
/* -------------------------------------------------------------------------- */
static int
lexicographically_less(const struct csfg_expr_pool* pool, int a, int b)
{
    if (pool->nodes[a].type == CSFG_EXPR_LIT &&
        pool->nodes[b].type == CSFG_EXPR_LIT)
        return pool->nodes[a].value.lit < pool->nodes[b].value.lit;

    if (pool->nodes[a].type == CSFG_EXPR_LIT)
        return 1;

    if (pool->nodes[a].type == CSFG_EXPR_VAR &&
        pool->nodes[b].type == CSFG_EXPR_VAR)
    {
        ...
        return strview_lexicographically_less(sa, sb);
    }
...
static int compare_and_swap(struct csfg_expr_pool* pool, int n)
{
    ...
    {
        if (lexicographically_less(pool, op, c))
            return 0;
    ...
    if (lexicographically_less(pool, b, c))
        return 0;
