...
TEST_F(NAME, light)
{
     ...
     *   s-1   a+4
     */
    int expr = csfg_expr_parse(&p, cstr_view("1/(1/(s-1)-1/(a+4))"));
    ASSERT_GE(expr, 0);
...
TEST_F(NAME, medium)
{
     ...
     */
    int expr = csfg_expr_parse(&p, cstr_view("1/(1/(s-1)^2-1/(a+4))"));
    ASSERT_GE(expr, 0);
    csfg_rule_remove_useless_ops(&p);
    csfg_rule_fold_constants(&p);
    expr = csfg_expr_gc(p, expr);
...
{
    int expr =
        csfg_expr_parse(&p, cstr_view("1/(1/s^3 - 4/(1+s)^2 - 8/(a+4))"));
    ...
    vt->set("a", 7.2);
    ASSERT_EQ(tfc.numerator.size(), 6u);
    ASSERT_EQ(tfc.denominator.size(), 6u);
...
TEST_F(NAME, compute_transfer_function)
{
    ...
    ASSERT_EQ(tf.roots(), 5);
    ASSERT_EQ(tf.poles(), 5);
#endif
}
...
TEST_F(NAME, inverting_amplifier)
{
    ...
    expr = csfg_graph_mason(&g, &p, paths, loops);
    ASSERT_GE(expr, 0);
