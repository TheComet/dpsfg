...
TEST_F(NAME, evaluate_with_variables)
{
    ...
    csfg_var_table_set_lit(&vt, "a", 2);
    csfg_var_table_set_lit(&vt, "c", 4);
    csfg_var_table_set_lit(&vt, "d", 2);
    csfg_var_table_set_lit(&vt, "e", 5);
...
TEST_F(NAME, generate_variable_table)
{
    ...
    EXPECT_THAT(csfg_var_table_eval(&vt, "a"), DoubleEq(0));
    EXPECT_THAT(csfg_var_table_eval(&vt, "b"), DoubleEq(0));
    EXPECT_THAT(csfg_var_table_eval(&vt, "c"), DoubleEq(1));
    EXPECT_THAT(csfg_var_table_eval(&vt, "d"), DoubleEq(1));
}
