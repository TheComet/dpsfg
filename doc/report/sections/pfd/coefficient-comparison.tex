...
Another way to achieve this is to bring everything to a common denominator,
factor in $A_n$ and compare the coefficients $b_n$ with the coefficients
$A_n,p_n$ to set up a linear system of equations.
...
\begin{equation}
    \frac{A_1}{(s-p_1)}\cdot\frac{s-p_2}{s-p_2}
        + \frac{A_2}{(s-p_2)}\cdot\frac{s-p_1}{s-p_1}
    = \frac{A_1s - A_1p_2 + A_2s - A_2p_1}{(s-p_1)(s-p_2)}
\end{equation}
...
\subsubsection{Second Order Example with Repeating Roots}

\begin{align}
    T(s) &= \frac{b_0 + b_1s}{(s-p)^2}
            = \frac{A_1}{(s-p)^2} + \frac{A_2}{s-p} \\
         &= \frac{A_1}{(s-p)^2} + \frac{A_2}{s-p} \cdot \frac{s-p}{s-p}
            = \frac{A_1 + A_2s - A_2p}{(s-p)^2} \\
         &= \frac{(A_1 - A_2p) + (A_2)s}{(s-p)^2}
\end{align}
...
\subsubsection{Fourth Order Example}
...
\begin{align}
  T(s) &= \frac{(s-8)(s-7)(s-5)(s+4)}{(s-1)^4(s+2)}
       ...
       &= \frac{A_1}{(s-1)^4}
          ...
          + \frac{A_4}{s-1}
          + \frac{A_5}{s+2} \\
       &= \frac{A_1(s+2) + A_2(s-1)(s+2) + A_3(s-1)^2(s+2) + A_4(s-1)^3(s+2) + A_5(s-1)^4}{(s-4)^4(s+2)}
\end{align}
